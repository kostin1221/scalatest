...
object MetrikaTaskProgress extends LazyLogging {
  ...
  def getLogRequestWhenReady(metrikaLogRequest: MetrikaLogGet)(implicit metrikaClient: MetrikaClient, timer: Timer[IO]): IO[MetrikaLogRequest] = {
    for {
      ...
      logRequest <- logRequest.status match {
        case Processed => IO(logger.debug(s"Request ${logRequest.requestId} are ready to provide the data")) *> IO.pure(logRequest)
        case Created => IO(logger.info(s"Request ${logRequest.requestId} has not yet proceeded, start sleeping until the next attempt")) *> timer.sleep(retryTimeout) *> getLogRequestWhenReady(metrikaLogRequest)
        case otherStatus => IO.raiseError(
          new LogRequestIsInWrongStatus(s"Log request is in ${otherStatus.toString} status so no data is available")
        )
      }
    } yield (logRequest)
  }
