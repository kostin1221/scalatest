...
import statistics.metrika.db.{DbStreamer, RequestStatusStorage}
...
object MetrikaDataStreamer extends LazyLogging {
  ...
  def createTaskAndStreamWhenReady(downloadStatisticsTask: DownloadStatisticsTask)(
    implicit metrikaClient: MetrikaClient,
    cs: ContextShift[IO],
    timer: Timer[IO],
    transactor: HikariTransactor[IO]
    ...
    for {
       getLogRequest <- MetrikaTaskCreator.createMetrikaTask(downloadStatisticsTask).map(
        createdLogRequest => mkGetLogRequest(downloadStatisticsTask, createdLogRequest)
      )
...
//       )
      ...
      preparedLogRequest <- MetrikaTaskProgress.getLogRequestWhenReady(getLogRequest)
  ...
  private def streamData(downloadStatisticsTask: DownloadStatisticsTask, metrikaLogRequest: MetrikaLogRequest)(
    implicit metrikaClient: MetrikaClient,
    cs: ContextShift[IO]
  ) = {
  ...
  private def mkGetLogRequest(downloadStatisticsTask: DownloadStatisticsTask, createdLogRequest: MetrikaLogRequest) = {
    MetrikaLogGet(downloadStatisticsTask.accessToken, downloadStatisticsTask.counterId, createdLogRequest.requestId)
  }
}
