...
class MetrikaClient(implicit cs: ContextShift[IO]) extends LazyLogging {
  ...
  def getListOfLogRequests(
    request: MetrikaLogGet
  ): IO[Response[Either[ResponseError[circe.Error], LogRequestsResponse]]] = {
    AsyncHttpClientFs2Backend[IO]().flatMap { implicit backend =>
      val response =
        createRequest(request)
          .get(
            uri"$BaseUrl/management/v1/counter/${request.counterId}/logrequests"
          )
  ...
  private def createRequest(baseMetrikaRequest: BaseMetrikaRequest) = {
    basicRequest.auth
      .bearer(baseMetrikaRequest.accessToken)
      .readTimeout(Duration(5, SECONDS))
  }

  def getLogRequest(
    request: MetrikaLogGet
  ): IO[Response[Either[ResponseError[circe.Error], LogRequestResponse]]] = {
    AsyncHttpClientFs2Backend[IO]().flatMap { implicit backend =>
      val response =
        createRequest(request)
          .get(
            uri"$BaseUrl/management/v1/counter/${request.counterId}/logrequest/${request.requestId}"
          )
  ...
  def createLogRequest(
    request: MetrikaLogCreate
  ): IO[Response[Either[ResponseError[circe.Error], LogRequestResponse]]] = {
    AsyncHttpClientFs2Backend[IO]().flatMap { implicit backend =>
      createRequest(request)
        .post(
        ...
        .body(
          Map[String, String](
            "date1" -> request.date1,
            "date2" -> request.date2,
            "fields" -> request.fields.mkString(","),
...
//          .map(logError)
  ...
  def streamLogRequestPartData(
    request: MetrikaLogStreamPart
  ): IO[Stream[IO, (Array[String], Array[String])]] = {
    AsyncHttpClientFs2Backend[IO]().flatMap { implicit backend =>
      createRequest(request)
        .get(
          uri"$BaseUrl/management/v1/counter/${request.counterId}/logrequest/${request.requestId}/part/${request.partNumber}/download"
        )
        ...
        .flatMap(resp => {
          resp.body match {
            ...
            case Left(body) =>
              IO.raiseError(
                new Exception(
                  s"Failed to get a stream, resp: ${body}, resp code: ${resp.code}"
                )
              )
          }
        })
      ...
      s.pull.uncons.flatMap {
        case Some((hd: Chunk[O], s: Stream[F, O])) =>
          val (headItem, tailChunks) = head match {
            case None =>
              val (head, tail: Chunk[O]) = hd.splitAt(1)
              (head.last.get, tail)
